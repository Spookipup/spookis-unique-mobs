...
// keeps the delta ghast in sightlines and beam range
public class DeltaGhastMaintainSightGoal extends Goal {

	private final DeltaGhastEntity ghast;

	private static final double PREFERRED_MIN_DIST = 20.0;
	private static final double PREFERRED_MAX_DIST = 40.0;
	...
	public DeltaGhastMaintainSightGoal(DeltaGhastEntity ghast) {
		this.ghast = ghast;
	...
	private void tickWithSight(double dist) {
		...
		double targetY = this.target.getY() + 8.0;
	...
	private void tickWithoutSight(double dist) {
		if (this.losLostTicks < LOS_LOST_PATIENCE) {
			return;
		}
