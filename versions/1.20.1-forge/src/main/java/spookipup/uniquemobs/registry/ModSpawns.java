...
public class ModSpawns {
	...
	private static final DeferredRegister<Codec<? extends BiomeModifier>> BIOME_MODIFIER_SERIALIZERS =
		DeferredRegister.create(ForgeRegistries.Keys.BIOME_MODIFIER_SERIALIZERS, UniqueMobs.MOD_ID);

	private static final RegistryObject<Codec<? extends BiomeModifier>> SPAWNS_BIOME_MODIFIER =
	...
	private static void registerPlacements(SpawnPlacementRegisterEvent event) {
	...
	private static <T extends Monster> void monsterPlacement(SpawnPlacementRegisterEvent event, EntityType<T> type) {
		event.register(type,
			SpawnPlacements.Type.ON_GROUND,
			Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private static <T extends Monster> void endMonsterPlacement(SpawnPlacementRegisterEvent event, EntityType<T> type) {
		event.register(type,
			SpawnPlacements.Type.ON_GROUND,
			Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private static <T extends Monster> void depthBoostedPlacement(SpawnPlacementRegisterEvent event, EntityType<T> type) {
		event.register(type,
			SpawnPlacements.Type.ON_GROUND,
			Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
			(entityType, level, spawnReason, pos, random) ->
	...
	private static <T extends Blaze> void blazePlacement(SpawnPlacementRegisterEvent event, EntityType<T> type) {
		event.register(type,
			SpawnPlacements.Type.ON_GROUND,
			Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
			Monster::checkMonsterSpawnRules,
	...
	private static <T extends Ghast> void lingGhastPlacement(SpawnPlacementRegisterEvent event, EntityType<T> type) {
		event.register(type,
			SpawnPlacements.Type.NO_RESTRICTIONS,
			Heightmap.Types.MOTION_BLOCKING,
	...
	private record UniqueMobsSpawnBiomeModifier() implements BiomeModifier {
		private static final Codec<UniqueMobsSpawnBiomeModifier> CODEC =
			Codec.unit(UniqueMobsSpawnBiomeModifier::new);
