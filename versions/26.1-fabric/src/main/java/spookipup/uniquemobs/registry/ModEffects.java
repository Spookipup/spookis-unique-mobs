...
public class ModEffects {
	...
	public static void addBlight(LivingEntity target, int stacks) {
		...
		if (totalStacks >= 2) {
			target.addEffect(new MobEffectInstance(MobEffects.SLOWNESS, 60 + totalStacks * 15, 0));
		}
		if (totalStacks >= 3) {
			target.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 80 + totalStacks * 20, 0));
		}
		if (totalStacks >= 4) {
			target.addEffect(new MobEffectInstance(MobEffects.NAUSEA, 50, 0));
		}
	}
