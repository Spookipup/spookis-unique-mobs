...
public class StormArcEntity extends Entity {
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		builder.define(DATA_END_OFFSET_X, 0.0F);
		builder.define(DATA_END_OFFSET_Y, 0.0F);
		builder.define(DATA_END_OFFSET_Z, 0.0F);
	...
	protected void readAdditionalSaveData(ValueInput input) {
		this.setEndOffset(new Vec3(
			input.getFloatOr("EndOffsetX", 0.0F),
			input.getFloatOr("EndOffsetY", 0.0F),
			input.getFloatOr("EndOffsetZ", 0.0F)
		));
		this.setSeed(input.getIntOr("Seed", 0));
	}

	@Override
	protected void addAdditionalSaveData(ValueOutput output) {
		Vec3 endOffset = this.getEndOffset();
		output.putFloat("EndOffsetX", (float) endOffset.x);
		output.putFloat("EndOffsetY", (float) endOffset.y);
		output.putFloat("EndOffsetZ", (float) endOffset.z);
		output.putInt("Seed", this.getSeedValue());
	}
