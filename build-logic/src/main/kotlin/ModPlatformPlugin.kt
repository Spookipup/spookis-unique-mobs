...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
	...
	private fun Project.configureProject(extension: ModPlatformExtension) {
		...
		val projectVersion = "$modVersion$channelTag+$mcVersion+$loader"
			...
			if (isForge) {
				manifest.attributes(
					"MixinConfigs" to listOf(
						...
						"${modId}.compat.betterdungeons.mixins.json",
						"${modId}.compat.betterfortresses.mixins.json"
					).joinToString(",")
				)
			}
		}
	...
	private fun Project.configureProcessResources(
		...
		tasks.named<ProcessResources>("processResources") {
			...
			val dependencies = buildDependenciesBlock(isFabric, loaderModId, extension.dependencies)
			val iconFile = rootProject.file("src/main/resources/assets/$modId/icon.png")
			if ((isNeoForge || isForge) && iconFile.exists()) {
				from(iconFile) {
					into("")
				}
			}
			...
			when {
				...
				isNeoForge -> {
					...
					exclude("META-INF/mods.toml", "fabric.mod.json", "aw/*.accesswidener", ".cache")
				}
