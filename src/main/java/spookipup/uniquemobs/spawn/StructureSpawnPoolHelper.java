...
public final class StructureSpawnPoolHelper {
	...
	private static boolean matchesResolvedStructure(StructureManager structureManager,
													BlockPos pos,
													Structure structure,
													java.util.function.Predicate<net.minecraft.world.level.levelgen.structure.StructureStart> predicate) {
		if (structure == null) {
			return false;
		}

		Map<Structure, it.unimi.dsi.fastutil.longs.LongSet> structuresAtPos = structureManager.getAllStructuresAt(pos);
		it.unimi.dsi.fastutil.longs.LongSet references = structuresAtPos.get(structure);
