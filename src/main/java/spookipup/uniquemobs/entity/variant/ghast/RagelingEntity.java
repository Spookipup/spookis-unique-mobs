...
public class RagelingEntity extends Ghast {
	...
	private static final int MAX_LIFETIME = 300; // 15 seconds - too angry to live longer
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		...
		builder.define(DATA_IS_FUSING, false);
	...
	private void explode() {
		...
		for (LivingEntity target : targets) {
			...
			target.hurtServer(serverLevel, fireDamage, damage);
			target.igniteForSeconds(FIRE_SECONDS);
		}

		this.playSound(SoundEvents.GENERIC_EXPLODE.value(), 1.5F, 1.2F);
	...
	public void playerTouch(Player player) {
		...
		if (this.level() instanceof ServerLevel serverLevel) {
			player.hurtServer(serverLevel, this.damageSources().onFire(), CONTACT_DAMAGE);
